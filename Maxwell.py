...
def pde(u, v):
   ...
   # Returns the real and imaginary residuals
    return [fEz, gEz]
...
def run_bayesian_optimization(initial_points=10, total_iterations=20, batch_size=10, random_seed=1234, 
                             custom_pbounds=None, verbose=2, callback=None):
    ...
    pbounds = custom_pbounds if custom_pbounds is not None else default_pbounds
    ...
    if initial_points > 0:
        ...
        lhs_samples = sampler.random(n=initial_points)
    ...
    num_batches = max(1, int(total_iterations / batch_size))
    for i in range(1, num_batches + 1):
        # For the last batch, calculate the remaining iterations
        current_batch_size = batch_size if i < num_batches else total_iterations - (i-1) * batch_size
        
        if current_batch_size > 0:
            optimizer.maximize(init_points=0, n_iter=current_batch_size)
            current_best = optimizer.max
            best_params = current_best["params"]
            best_error = -current_best["target"]
            print(f"Batch {i}/{num_batches} - Best Error: {best_error}")
...
#-------------------------------------------------------------------------------#
class RarDSampler:
    def __init__(self, model, geom, data, pde, tol=1e-3, num_iterations=10, num_new_points=50):
        self.model = model  
        self.geom = geom  
        self.data = data 
        self.pde = pde  
        ...
        self.num_new_points = num_new_points
    ...
    def filter_unique_points(self, existing_points, new_points):
        ...
        for p in new_points:
            ...
            # If the minimum distance to any existing point is greater than tol, the point is unique
    ...
    def sample_points(self):
        """
        Performs RAR-D adaptive sampling by iterating through the specified number of iterations.
        """
        for i in range(self.num_iterations):
            ...
            residuals_pde = self.model.predict(candidate_points, operator=self.pde)

            # Calculate the combined residual for each point
            residuals_combined = np.sqrt(np.array(residuals_pde[0])**2 + np.array(residuals_pde[1])**2).ravel()

            # Constructing probability distributions for selecting new points
            ...
            new_points_unique = self.filter_unique_points(existing_points, new_points)

            # Add unique points to the training set
            if new_points_unique.shape[0] > 0:
                self.data.add_anchors(new_points_unique)
            else:
...
def train_and_evaluate_with_dynamic_weights():
    ...
    initial_points=10,     # 10 initial points using LHS
    total_iterations=20,   # 20 total iterations
    batch_size=10,         # Report every 10 iterations
    random_seed=42,        # Custom random seed
    custom_pbounds={       # Custom parameter bounds
        'w_pde': (0.01, 0.10),
        'w_abc': (0.01, 0.25),
        'lr': (0.001, 0.01),
        'num_domain_idx': (10, 100),
    ...
    # Create the RarDSampler object
    rar_sampler = RarDSampler(model=model, geom=geom, data=data, pde=pde, tol=1e-3, num_iterations=10, num_new_points=50)

    # Run the adaptive sampling
    rar_sampler.sample_points()
